# Farm Cleaner

Stardew Valley mod for cleaning the farm without hours of manual effort.

![cleaner](./img/cleaner.jpg)

Just press the hot key (*default*: `K`) and wait for the debris to be collected. By default this only works on your farm, but non-farm areas can be enabled in the settings. You can change it in [GMCM (Generic Mod Config Menu)](https://www.nexusmods.com/stardewvalley/mods/5098). For Android players, you'll need [VirtualKeyboard](https://www.nexusmods.com/stardewvalley/mods/29884) mod.

## Settings

This mod provides the following settings.

![settings](./img/settings.png)

### Hot Key

The key to clear selected trees, stones, grass, and debris from the current location. Defaults to `K`.

### Enable On Non-Farm Areas

Whether to allow clearing the current location outside the farm. Defaults to disabled. Use with care on custom maps.

### Gain Experience

Whether to gain experience from clearing the farm. Defaults to enabled. When disabled, no experience is granted.

### Clear Grass

Whether to clear grass. Defaults to disabled — grass will not be cleared.

### Clear Fruit Trees

Whether to clear fruit trees. Defaults to disabled — fruit trees will not be cleared.

### Clear Tapped Trees

Whether to clear trees that have a tapper attached. Defaults to disabled — tapped trees will not be cleared.

### Clear Growing Trees

Whether to clear trees that are not yet fully grown. Defaults to disabled — saplings and young trees will not be cleared.

### Clear Giant Crops

Whether to clear giant crops and meteorites. Defaults to disabled — giant crops and meteorites will not be cleared.

### Drop Multiplier

A multiplier applied to the number of dropped items (wood, stone, fiber, etc). Defaults to 1.0. Range: 0.1 – 10.0.

This is extremely useful when you don't want to collect the basic resources repeatedly, and it's not that balance-breaking in my opinion.

When your backpack is full, the extra items will be dropped near you after all debris is collected.

![overflow](./img/overflow.jpg)
